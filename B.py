...
# Optin
    ...
    # Load assets
    local_css("style.css")
    lottie_duck = load_lottiefile(("animation/Duck.json"))
...
if st.session_state.stage == 3:
    ...
    with col2:
        if not name_customer:
            ...
            cursor.execute("SELECT * FROM Orders WHERE OrderCode=?", (name_customer,))
...
if st.session_state.stage == 4:
    ...
    if order_data:
        ...
        cursor.execute("INSERT INTO Orders (OrderCode, Product, TotalPrice, CustomerNote) VALUES (?, ?, ?, ?)",
                       (order_data['customer_name'], str(product_quantities), int(order_data['total_price']), str(order_data["customer_note"])))
        # Commit the changes
        conn.commit()

        col1, col2 = st.columns(2)
        col1.button('กลับหน้าหลัก', key="back1",on_click=lambda: (set_state(1), st.session_state.update(cart={})), use_container_width=True)
...
if st.session_state.stage == 5:
    ...
    col1.button('กลับหน้าหลัก', key="back2",on_click=lambda: (set_state(1), st.session_state.update(cart={})), use_container_width=True)
    if col2.button('ตรวจสอบสถานะอีกครั้ง',on_click=set_state, args=[5], use_container_width=True):
        ...
        if ordercode:
            ...
            cursor.execute("SELECT * FROM Orders WHERE OrderCode=?", (ordercode,))
            result = cursor.fetchone()
            ...
            if result:
                ...
                incorrect_gif = load_lottiefile("animation\incorrect.json")
                st_lottie(
                    incorrect_gif,
                    speed=2,
                    reverse=False,
                ...
                #insert correct GIF
                correct_gif = load_lottiefile("animation/correct.json")
                st_lottie(
                    correct_gif,
                    speed=2,
                    reverse=False,
...
if st.session_state.stage == 6:
    ...
    col1, col2 = st.columns(2)
    col1.button('กลับหน้าหลัก', key="back3",on_click=lambda: (set_state(1), st.session_state.update(cart={})), use_container_width=True)
    if not name_chek :
        ...
        if col2.button('ตรวจสอบสถานะอีกครั้ง',on_click=set_state, args=[5], use_container_width=True):
            ...
            if name_chek:
                # Check if the product code exists in the database.
                cursor.execute("SELECT * FROM Orders WHERE OrderCode=?", (name_chek,))
                result = cursor.fetchone()
